...
export default function ReducedProjectComponent({ project }) {
  ...
  const { error, imagesArePending, data: imageURLs } = useFetch(`${process.env.REACT_APP_PROTOCOL_DOMAIN}/api/images/${project_id}/`);
  const { error2, isPending, data: user } = useFetch(`${process.env.REACT_APP_PROTOCOL_DOMAIN}/api/users/${user_id}`);
